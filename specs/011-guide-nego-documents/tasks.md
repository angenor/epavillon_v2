...
- [ ] T016 [P] Rendre `altText` facultatif dans `UploadPayload` (`frontend/app/types/media.ts`) quand le type n'est pas une image, et le vérifier côté serveur au premier dépôt d'un PDF. Si le serveur l'exige pour tout type, le corriger dans `media/src/service/upload.rs`, avec son test
- [ ] T017 [P] Créer `frontend/app/components/media/FileField.vue` sur `useDepotMedia` : choix du fichier, type accepté en prop, progression, erreurs, émissions `update:assetId`. Pas de recadrage ni de texte alternatif. L'ajouter à `frontend/app/pages/style-guide.vue`
- [ ] T018 Écrire la requête qui compte les objets `private` déjà déposés dans le bucket public (dette de R4), dans `specs/011-guide-nego-documents/quickstart.md` § Préalables, et la jouer en local
