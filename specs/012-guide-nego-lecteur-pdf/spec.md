...
**Status**: Draft — relue par le commanditaire le 24/09 : trois hypothèses tranchées (voir *Les arbitrages du 24/09*) ; **après l'essai, quatre décisions** (voir *Les arbitrages après l'essai*)
...
| Les arbitrages après l'essai, 24/09 | L'essai ([essai-lecteur.md](essai-lecteur.md)) rend l'issue A ; [ADR-022](../../docs/AppNego/adr/022-pdfjs-dans-le-client.md) les consigne. (1) **Sur un réseau très lent, la première page de ce guide met environ 25 s**, la couverture étant une photo de 396 Ko : accepté, mais **l'attente se voit** — progression, puis au bout de 3 s « Lire le texte en attendant » et « Télécharger pour lire sans réseau » (FR-009 bis) ; une couverture allégée est écartée ; la bascule ne sanctionne jamais le réseau (FR-012 bis) ; SC-003 réécrit. (2) **iOS 16.4 et au-delà affichent les pages**, grâce à un remplacement de `Promise.withResolvers` dans la page et dans le travailleur ; en dessous, « Texte agrandi ». (3) **Les polices standard de pdf.js sont gardées**, les quatre Liberation comprises : un autre document peut ne pas embarquer ses polices. (4) Les trois défauts trouvés à l'essai vont au plan |
...
| Les décisions | [ADR-003](../../docs/AppNego/adr/003-tout-ce-qui-se-lit-se-lit-hors-connexion.md) tout ce qui se lit se lit hors connexion · [ADR-019](../../docs/AppNego/adr/019-service-worker-engendre-cache-d-abord.md) les documents vivent hors des caches de la coquille · [ADR-021](../../docs/AppNego/adr/021-pdfium-dans-le-worker.md), dont la règle « le PDF ne va jamais sur le téléphone » **tombe** : [ADR-022](../../docs/AppNego/adr/022-pdfjs-dans-le-client.md) la remplace · [ADR-012](../../docs/AppNego/adr/012-toute-source-porte-un-etat.md) la note de correction sur une page |
...
8 bis. **Given** un document non téléchargé et un réseau très lent, **When** la première page tarde, **Then** l'attente montre ce qui est reçu sur ce qui est demandé ; au bout de 3 s, elle offre « Lire le texte en attendant » et « Télécharger pour lire sans réseau » ; la page du PDF prend la place du texte quand elle est prête, sauf si la personne a choisi de rester sur le texte.
...
- **Un réseau très lent.** La première page de ce guide met 23 à 28 s en « 3G lente » : sa couverture est une photo de 396 Ko. L'attente se voit, le texte se lit en moins de 5 s (FR-009 bis), et le lecteur ne bascule pas à cause du réseau (FR-012 bis).
...
- **Un téléphone trop ancien pour afficher les pages** — un iPhone sous iOS 16.4 : pdf.js y est refusé en entier. L'iPhone 8 et le X, en iOS 16.7, affichent les pages (essai, confirmé à T084). Il lit « Texte agrandi » quand le document l'offre, et le lecteur le dit ; sinon l'écran dit que ce téléphone ne peut pas afficher ce document (FR-012 bis).
...
- **FR-009 bis** : Tant que la première page d'un document lu en ligne n'est pas dessinée, l'attente DOIT montrer sa progression — ce qui est reçu sur ce qui est demandé. Au bout de 3 s sans page, elle DOIT offrir « Lire le texte en attendant » — « Texte agrandi », à la même page, quand le document l'offre — et « Télécharger pour lire sans réseau ». La page du PDF DOIT prendre la place du texte quand elle est prête, **sauf si la personne a choisi de rester sur le texte**. Le texte du document DOIT se lire avant le PDF, pour ne pas partager le débit avec lui (arbitré le 24/09, après l'essai).
...
- **SC-003** : La première page d'un document est lisible en moins de trois secondes sur un réseau ordinaire, avant que le fichier entier soit arrivé ; **le seuil de huit secondes vaut sur un réseau ordinaire et depuis une copie gardée**. **Sur un réseau très lent** (« 3G lente »), une première lecture est possible **en moins de cinq secondes, par le texte** ; la page du PDF suit quand elle est reçue (arbitré le 24/09, après l'essai : la couverture du guide pèse à elle seule 8 s de ce débit).
