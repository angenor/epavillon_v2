...
**Dernière mise à jour** : 16 août 2026 — alignement sur le guide de style de référence, et image de couverture des activités.
...
| Sujet | Nature | Depuis |
|-------|--------|--------|
| **`v_public_schedule` n'expose ni le pays de l'organisation, ni les libellés et couleurs des thématiques** — la carte de séance a besoin des deux, et la vue est censée répondre à l'écran en une requête (écarts n° 14 et 15). Contourné par des propriétés séparées dans `UiSessionCard`. **La troisième lacune de la même famille — l'image — a été corrigée le 16/08** ; ces deux-là restent | Correction du SQL à décider **avant A3**. Les deux se traitent dans la même passe sur la vue, sur le modèle de ce qui a été fait pour `cover` | 2026-08-16 |
