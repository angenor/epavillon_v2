...
| Outillage de la phase B | ✅ Installé le 20/08 — GitHub Spec Kit 0.16.5, intégration `claude`, scripts `sh`. `.specify/` (gabarits, scripts, workflow, constitution) et `.claude/skills/speckit-*` (dix compétences). Constitution encore à l'état de gabarit |
...
Le tableau et les obligations d'API relevées en écrivant les écrans : [`progression/api.md`](progression/api.md). **Aucun prompt B n'est commencé** ; seul l'outillage Spec Kit est posé (20/08). Le prochain pas est **B0 — la constitution**, `/speckit-constitution`.
