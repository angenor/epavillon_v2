...
**Dernière mise à jour** : 16 août 2026 — types TypeScript dérivés du modèle (prompt A0.2).
...
| Front | 🟨 Socle posé le 16/08 — Nuxt 4.5, TypeScript strict, Tailwind v4, Pinia, i18n fr/en, jetons de design, deux layouts. **Types du modèle dérivés le 16/08** : 14 fichiers, 62 tables du jalon, 2 vues, 8 retours de fonction. `make check-front` au vert. Aucun écran encore |
...
| A0.2 | Types TypeScript dérivés du SQL | ✅ | Fait le 16/08. Périmètre du jalon : `reference`, `identity`, `org`, `media`, `event`, `programme`, les deux vues. Modules hors jalon (Direct, Publications, Négociations, Engagement, Formations, Outils, Analytique) non couverts — ils viendront avec leurs écrans |
...
Neuf points où le modèle et ce que l'interface demanderait naturellement ne coïncident pas. **Aucun n'a été corrigé** : ni le SQL, ni les types ne compensent. Ils se trancheront un par un, au moment où l'écran concerné les rencontrera.

| Écart | Où | Ce qu'il coûte à l'interface | À trancher |
|---|---|---|---|
| **La vue du back-office renvoie un titre DÉJÀ RÉSOLU** — `v_proposal_dashboard.title` vaut `platform.t(p.title)`, un `text`, là où `proposals.title` est un `i18n_text` | `070` § 7 | Le même champ porte deux types selon sa source. La liste des propositions ne peut pas changer de langue sans requête, et un développeur qui applique `resolveI18nText()` dessus obtient une chaîne vide sans erreur | La vue renvoie-t-elle le JSON brut, ou l'interface assume-t-elle le rechargement ? |
| **Le nom d'une organisation n'est pas multilingue** — `org.organizations.legal_name` est un `text` | `040` § 1 | La programmation publique en anglais affiche un nom légal français. Les traductions existent (`organization_names`, `kind = 'translation'`) mais ni `v_public_schedule` ni la fiche ne les joignent | Joindre la dénomination traduite dans la vue, ou assumer le nom légal partout ? |
| **Les thématiques d'une proposition sont hors de la proposition** — elles vivent dans `reference.entity_terms`, polymorphe et sans clé étrangère vers `proposals` | `020` § 4 | Aucun type ne peut relier `entity_id` à `Proposal.id` : le formulaire de soumission fait un aller-retour sur une table générique, et rien ne l'empêche d'écrire un mauvais couple `(schéma, table)` | Prix assumé du transversal, ou table de liaison dédiée dans `programme` ? |
| **La machine à états est une donnée que rien n'expose** — `proposal_transitions_allowed` dit quelles actions sont possibles, mais aucune vue ne la joint à la proposition | `070` § 1 | Sans point d'accès dédié, le front réimplémentera le graphe des transitions en dur : exactement ce que la mise en données voulait éviter | Prévoir `GET /proposals/:id/transitions` au prompt B4 |
| **`administered_events()` a trois réponses pour une question binaire** — la fonction agrège sans `GROUP BY` : elle renvoie toujours une ligne, et sans aucune attribution `is_global` **et** `event_ids` valent `null` | `030` § 3 | Un garde de navigation qui teste `if (!scope.is_global)` traite « aucun droit » comme « administrateur d'événement » et affiche une liste vide au lieu d'un refus | Normaliser côté API (`false` / `[]`), ou traiter `null` explicitement dans chaque garde |
| **Les réponses d'inscription ne sont pas typables** — `registrations.answers` est un `jsonb` dont les clés sont les `code` de `registration_form_fields` | `075` § 4 | `Record<string, unknown>` est le seul type honnête ; toute la validation est dynamique, contre le formulaire chargé | Conséquence voulue du formulaire configurable. Rien à trancher, mais à ne pas « améliorer » par un type figé |
| **Trois colonnes de `sessions` sont écrites par trigger** — `time_range`, `enforce_room_exclusivity`, et `broadcast_channel_id` quand `is_streamed` passe à vrai | `075` § 1 | Un formulaire qui les envoie verra ses valeurs écrasées sans avertissement. Marquées `readonly` dans les types pour les deux premières ; la troisième ne peut pas l'être, elle est saisissable | Que l'API les ignore en écriture plutôt que de les accepter en silence |
| **Aucune vue « mes propositions »** — l'espace organisation (A5) et la page publique d'une session (A3) devront composer leurs jointures | `070`, `075` | Deux écrans du jalon n'ont pas d'équivalent de `v_proposal_dashboard` ni de `v_public_schedule` : le N+1 est à éviter côté API, pas côté front | Ajouter deux vues, ou traiter la jointure dans le module Rust ? |
| **Le numéro de dossier dépend d'un champ facultatif** — `reference_code` est préfixé par `events.acronym`, nullable, avec repli sur les huit premiers caractères du slug | `070` § 2 | Une édition sans sigle donne un préfixe tronqué et pénible à épeler au téléphone : slug `cop31-belem-2027` → dossiers « COP31-BE-00147 » | Rendre `acronym` obligatoire à la création d'une édition, côté formulaire d'administration (A10) |
