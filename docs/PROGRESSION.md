...
**Dernière mise à jour** : 16 août 2026 — traitement des neuf écarts relevés en dérivant les types.
...
| Front | 🟨 Socle posé le 16/08 — Nuxt 4.5, TypeScript strict, Tailwind v4, Pinia, i18n fr/en, jetons de design, deux layouts. **Types du modèle dérivés le 16/08** : 14 fichiers, 62 tables du jalon, 2 vues, 8 retours de fonction, puis alignés le 16/08 sur les deux corrections du modèle (`AdministeredEvents`, `ProposalDashboardRow`). `make check-front` au vert. Aucun écran encore |
