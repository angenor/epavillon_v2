...
export function useApi() {
  ...
  // Le bloc `auth` vit dans `api/auth.ts` depuis le 22/09. Déclaré ici comme
  // constante et non dans le littéral de retour : le bloc de Guide Négo le
  // réemprunte tel quel, pour y joindre son objet `client` en un seul endroit.
  const auth = createAuthApi(deps)
