...
// Tiré de `zone.tab` et des liens de `tzdata.zi` — la base de fuseaux de PostgreSQL,
// qui n'expose pas le pays d'un fuseau. Donnée IANA publique, non métier.
const ZONES_BY_COUNTRY: Record<string, string> = {
  ...
  CH: 'Europe/Zurich',
  CI: 'Africa/Abidjan Africa/Timbuktu',
  ...
  GB: 'Europe/Belfast Europe/London',
  GD: 'America/Grenada',
  GE: 'Asia/Tbilisi',
  GF: 'America/Cayenne',
  GG: 'Europe/Guernsey',
  ...
  NR: 'Pacific/Nauru',
  NU: 'Pacific/Niue',
  NZ: 'Antarctica/South_Pole Pacific/Auckland Pacific/Chatham',
  OM: 'Asia/Muscat',
  PA: 'America/Coral_Harbour America/Panama',
  PE: 'America/Lima',
...
const COUNTRY_BY_ZONE = new Map<string, string>(
  Object.entries(ZONES_BY_COUNTRY).flatMap(([iso2, zones]) =>
    zones.split(' ').map((zone) => [zone, iso2] as const),
  ),
)

/** Code ISO 3166-1 alpha-2 du pays d'un fuseau ; `null` pour `Etc/…` et les zones océaniques. */
export function timeZoneCountryIso2(timeZone: TimeZoneName): string | null {
  return COUNTRY_BY_ZONE.get(timeZone) ?? null
}

// `Brazil/East`, `US/Eastern` : anciens noms, dont le dernier segment n'est pas une ville.
const CITY_AREAS = new Set([
  'Africa', 'America', 'Antarctica', 'Asia', 'Atlantic', 'Australia', 'Europe', 'Indian', 'Pacific',
])

/** Le dernier segment du fuseau est-il un nom de lieu ? */
export function hasCityName(timeZone: TimeZoneName): boolean {
  return CITY_AREAS.has(timeZone.split('/')[0] ?? '')
}
