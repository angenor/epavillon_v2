...
/**
 * Bascule de thème — le ciel jour/nuit.
 *
 * DEUX ÉTATS À L'ÉCRAN, TROIS DANS LE STORE. Le bouton est binaire (`aria-pressed`),
 * mais « système » reste le défaut du store : tant que personne n'a cliqué, la
 * plateforme suit la préférence du navigateur et le bouton se contente d'en
 * afficher le résultat. Le premier clic tranche explicitement et quitte le mode
 * système — c'est le prix d'une bascule à deux états, arbitré le 19/08.
 *
 * POURQUOI `ready`. En rendu serveur, « système » est indéterminable : le serveur
 * ignore le `prefers-color-scheme` du navigateur. Le bouton part donc en position
 * « jour », et `onMounted` le corrige. Sans garde, cette correction jouerait
 * l'animation complète au chargement de chaque page. `ready` n'autorise les
 * transitions qu'après deux trames, une fois la position juste déjà posée.
 *
 * LES ILLUSTRATIONS SONT DES ASSETS. Les deux ours pèsent 35 ko de tracés à eux
 * seuls ; inline, ils feraient sauter le garde-fou des 1000 lignes. Les nuages et
 * les étoiles, eux, restent dans le template : le CSS cible leurs `path` un à un
 * (échelle des étoiles, scintillement, voile du premier nuage), ce qu'un `<img>`
 * externe ne permettrait pas.
 */

interface Props {
  /** Version compacte, 40 px de haut : barres d'outils sur écran large uniquement. */
  compact?: boolean
}
...
/** Préférence déclarée par le navigateur. Faux au rendu serveur, faute de pouvoir la lire. */
const preferredDark = ref(false)
/** Les transitions n'entrent en jeu qu'une fois la position d'origine posée. */
const ready = ref(false)

let media: MediaQueryList | null = null

const isDark = computed(() =>
  preferences.theme === 'system' ? preferredDark.value : preferences.theme === 'dark',
)

const currentLabel = computed(() =>
  t('nav.theme.current', { theme: t(isDark.value ? 'nav.theme.dark' : 'nav.theme.light') }),
)

function readPreference(query: MediaQueryList | MediaQueryListEvent): void {
  preferredDark.value = query.matches
}

onMounted(() => {
  media = window.matchMedia('(prefers-color-scheme: dark)')
  readPreference(media)
  media.addEventListener('change', readPreference)
  // Deux trames : la première applique la position juste, la seconde ouvre les transitions.
  requestAnimationFrame(() => requestAnimationFrame(() => (ready.value = true)))
})

onBeforeUnmount(() => media?.removeEventListener('change', readPreference))
...
<template>
  <button
    type="button"
    class="toggle"
    :class="{ 'toggle--compact': props.compact, 'is-ready': ready }"
    :aria-pressed="isDark"
    :title="t('nav.theme.toggle')"
    @click="toggle"
  >
    <span class="sr-only">{{ t('nav.theme.toggle') }} — {{ currentLabel }}</span>
...
<style scoped>
/*
 * Les couleurs ci-dessous sont celles d'une ILLUSTRATION — un ciel, un soleil,
 * une lune —, pas des rôles d'interface. Elles n'ont donc pas de jeton
 * sémantique et ne s'inversent pas en thème sombre : le bouton porte lui-même
 * ses deux états. Seul l'anneau de focus emprunte `--color-focus` au projet.
 */
.toggle {
  ...
  cursor: pointer;
  flex: none;
  /* Contournement du débordement iOS : le rayon seul ne suffit pas à couper les tracés. */
  isolation: isolate;
  ...
  box-shadow:
    ...
    0 calc(var(--width) * 0.02) calc(var(--width) * 0.5) 0 hsl(210 10% 100% / 0.15);
}
...
/*
 * La cible tactile, détachée du dessin. `.toggle` ne clippe plus lui-même —
 * `.toggle__content` s'en charge déjà, par `overflow` ET `clip-path` — sans quoi
 * ce pseudo-élément serait rogné avec le reste et ne servirait à rien.
 */
.toggle::before {
  content: '';
  position: absolute;
...
/* L'original neutralisait l'anneau de focus ; on le rétablit avec le jeton du projet. */
.toggle:focus-visible {
  outline: 2px solid var(--color-focus);
  outline-offset: 3px;
}

/* Tant que la position juste n'est pas posée, aucune transition ne joue. */
.toggle:not(.is-ready),
.toggle:not(.is-ready) :deep(*) {
...
/* Les étoiles reviennent en élastique quand on repasse au jour. */
.toggle[aria-pressed='false'] .toggle__backdrop:last-of-type {
  transition-timing-function: cubic-bezier(0.2, -0.6, 0.7, 1.6);
}

.toggle[aria-pressed='false'] .stars path {
  transition-delay: 0s;
}
...
/* Trois étoiles sur onze scintillent, à contretemps. */
...
/* Le halo du soleil, qui se resserre en lune. */
.toggle__star::before {
  content: '';
  z-index: -1;
  width: 356%;
...
.toggle__star::after {
  ...
  filter: blur(calc(var(--width) * 0.033));
  translate: 2% 4%;
  border-radius: 50%;
  z-index: -1;
}
...
/* La lune ne se substitue pas au soleil : elle glisse par-dessus, à l'intérieur. */
.moon {
  position: absolute;
  inset: -1%;
  border-radius: 50%;
...
/* Les deux ours : chacun entre par un bord et sort par l'autre, au fil de --bear-speed. */
.pilot__container,
...
.pilot-bear {
  ...
  left: 100%;
  transition:
    translate calc(var(--speed) + ((1 - var(--dark, 0)) * ((var(--bear-speed) * 0.5) - var(--speed))))
...
/* L'original visait un `<svg>` inline ; les ours sont ici des assets, donc des `<img>`. */
.astrobear img {
  transform-origin: 50% 75%;
  scale: var(--dark, 0);
  rotate: calc(var(--dark, 0) * 360deg);
  transition:
    rotate calc(var(--speed) + (var(--dark, 0) * (var(--bear-speed) - var(--speed)))) calc(var(--bear-speed) * 0.4)
      linear,
...
/*
 * Vol de l'avion, scintillement et rotation de l'astronaute tournent en boucle :
 * ce sont exactement les mouvements que `prefers-reduced-motion` demande de taire.
 * La bascule, elle, reste instantanée plutôt que supprimée — l'état doit rester lisible.
 */
