...
for (const fichier of sources) {
  ...
  // Premier argument de call(…), send(…), sendForm(…), lireEtiquete(…),
  // ecrireEtiquete(…) et pending(…). Il n'est pas toujours un
  ...
  for (const m of src.matchAll(/\b(call|sendForm|send|lireEtiquete|ecrireEtiquete|pending)\s*(?:<[^(]*>)?\s*\(\s*([\s\S]{0,200}?)(?:,\s*(?:\(|async)|\)\s*$)/gm)) {
