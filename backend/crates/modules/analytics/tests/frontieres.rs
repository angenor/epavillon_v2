...
//!
//! **`live` en fait partie**, et c'est le point : le tableau de bord affiche les
//! messages actifs de l'édition, mais il les lit par une **fonction SQL**. Les
//! deux crates de ce jalon ne partagent aucune ligne de Rust.
...
fn le_manifeste_ne_cite_aucun_crate_de_module() {
    ...
    for module in [
        ...
        assert!(
            ...
            "le crate `analytics` ne dépend pas de `{module}`"
        );
    }
}
...
fn cargo_tree_ne_porte_aucune_arete_vers_un_module() {
    let sortie = Command::new(env!("CARGO"))
        .args(["tree", "-p", "analytics", "--edges", "normal,build,dev"])
