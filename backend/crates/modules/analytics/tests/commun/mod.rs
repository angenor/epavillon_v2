...
//! décor d'édition, des comptes, et **le rafraîchissement sans lequel tout
//! chiffre vaut zéro**.
//!
//! # LES PROJECTIONS SONT VIDES À LA CRÉATION
//!
//! `analytics.mv_*` sont des vues **matérialisées** : elles ne contiennent rien
//! tant que `refresh_all()` n'a pas tourné. **Tout test qui lit un chiffre
//! commence donc par `rafraichir(&bac).await`** — c'est une contrainte de
//! rédaction, pas un détail : sans elle, un test « passerait » sur une
//! projection vide et ne prouverait rien.
//!
//! Le premier peuplement se fait **hors mode concurrent** : `REFRESH …
//! CONCURRENTLY` refuse une vue jamais peuplée.
//!
//! # POURQUOI LA VRAIE APPLICATION N'EST PAS MONTÉE ICI
//!
//! La monter demanderait au crate `analytics` une dépendance de développement
//! vers `api`. Le contrôle de frontière du principe II, `cargo tree`, liste
//! aussi les dépendances de développement : l'arête le ferait échouer. Le
//! périmètre et la permission vivent donc dans `service/`, et les tests les
...
impl Bac {
    pub async fn monter() -> Self {
        ...
        let state = AnalyticsState::new(base.db(), config.clone());
        Self {
            base,
            state,
            config,
...
/// **À appeler avant toute lecture de chiffre.** Sans elle, les huit
/// projections sont vides et un test « passe » en ne prouvant rien.
///
/// Hors mode concurrent : `REFRESH … CONCURRENTLY` refuse une vue matérialisée
/// jamais peuplée, et c'est exactement l'état d'une base neuve.
pub async fn rafraichir(bac: &Bac) {
    sqlx::query!(r#"SELECT vue FROM analytics.refresh_all(false)"#)
        .fetch_all(bac.pool())
        .await
        .expect("rafraîchissement des projections");
}

/// Change le seuil d'urgence **en base**, comme le ferait l'IFDD d'une COP à
/// l'autre — sans redéploiement.
pub async fn poser_le_seuil(bac: &Bac, jours: i32) {
    sqlx::query!(
        "UPDATE platform.settings SET value = to_jsonb($1::text)
          WHERE key = 'analytics.review_alert_days'",
        jours.to_string()
    )
    .execute(bac.pool())
    .await
    .expect("écriture du seuil");
}

/// Supprime le réglage : la composition doit alors retomber sur son repli.
pub async fn supprimer_le_seuil(bac: &Bac) {
    sqlx::query!("DELETE FROM platform.settings WHERE key = 'analytics.review_alert_days'")
        .execute(bac.pool())
        .await
        .expect("suppression du seuil");
}

/// Un appel à propositions sur l'édition, dont l'échéance tombe dans `jours`.
pub async fn appel(bac: &Bac, event_id: Uuid, code: &str, jours: i64) -> Uuid {
    sqlx::query_scalar!(
        r#"INSERT INTO event.calls_for_proposals
               (event_id, code, title, status, opens_at, closes_at)
           VALUES ($1, $2, '{"fr":"Appel","en":"Call"}'::jsonb, 'open',
...
/// Un dossier déposé, sans aucune revue.
pub async fn dossier_depose(
    bac: &Bac,
    event_id: Uuid,
    call_id: Option<Uuid>,
