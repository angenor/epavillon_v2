...
//!
//! **Les appels de fonction ne comptent pas** : `analytics.refresh_all()` écrit
//! son journal et `analytics.enqueue_refresh()` dépose dans `platform.jobs`,
//! mais c'est le MODÈLE qui écrit — le module ne compose aucun `INSERT`. C'est
//! précisément pour cela que la mise en file passe par la fonction plutôt que
//! par `kernel::jobs::enqueue`.
...
fn aucune_ecriture_ne_vise_un_autre_schema() {
    ...
    assert!(
        fautes.is_empty(),
        "écritures hors du schéma `analytics` :\n{}",
        fautes.join("\n")
    );
}
